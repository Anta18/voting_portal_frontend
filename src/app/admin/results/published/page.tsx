...
interface FinalResultsPayload {
  ...
  results: {
    [candidate: string]: number | string; // Allow string numbers from backend
  };
}
...
export default function FinalResultsPage() {
  ...
  useEffect(() => {
    async function fetchSidebarElections() {
      try {
        ...
        const res = await fetch(`${API_URL}/election/admin/past`, {
          headers: {
    ...
    async function fetchFinalResults() {
      ...
      try {
        ...
        const res = await fetch(
          `${API_URL}/results/admin/final?election_id=${selectedElection}`,
          {
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${token}`,
        ...
        const data = await res.json();
        // Backend returns final_results as an object with election details
  ...
  // Adjusted: use finalResult.results only if the election_id matches the selectedElection
  let resultsForElection = null;
  if (finalResult && finalResult.election_id === selectedElection) {
    resultsForElection = finalResult.results;
  }
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
                  <div>
                    <h1 className="text-2xl lg:text-3xl font-bold">
                      {finalResult &&
                      finalResult.election_id === selectedElection
